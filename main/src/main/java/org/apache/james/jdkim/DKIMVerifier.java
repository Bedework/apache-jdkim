...
public class DKIMVerifier extends DKIMCommon {
    ...
    public PublicKeyRecord publicKeySelector(final List<String> records)
            throws PermFailException {
     ...
     */
    public List<SignatureRecord> verify(final InputStream is) throws IOException,
            FailException {
        Message message;
        try {
	        try {
     ...
     *                 if no signature can be verified
     */
    public List<SignatureRecord> verify(final Headers messageHeaders,
     ...
     */
    public List<SignatureRecord> verify(final BodyHasher bh) throws FailException {
        if (bh == null) {
			return null;
		}
        CompoundBodyHasher cbh = validateBodyHasher(bh);
     ...
     * @throws FailException if no valid signature is found
     */
    private List<SignatureRecord> verify(final CompoundBodyHasher compoundBodyHasher)
            throws FailException {
     ...
     */
    private FailException prepareException(final Map<String, FailException> signatureExceptions) {
    ...
    private void signatureVerify(final Headers h, final SignatureRecord sign,
            final byte[] decoded, final PublicKeyRecord key, final List<CharSequence> headers)
            throws PermFailException {
        try {
